                ...
                <ul class="portfolio-filters isotope-filters" data-aos="fade-up" data-aos-delay="100">
                    ...
                    @foreach ($categoryTypes as $type)
                        @foreach ($categories->where('type', $type) as $category)
                          <li data-filter=".filter-{{ $category->id }}-{{ Str::slug($category->type) }}">
                            {{ $category->name }} {{ $category->type == 'dengan_foto' ? 'Dengan Foto' : 'Tanpa Foto' }}
                          </li>
                        @endforeach
                    @endforeach
                </ul>
                      ...
                      <div class="col-lg-3 col-md-4 col-sm-6 portfolio-item isotope-item filter-{{ $template->category->id }}-{{ Str::slug($template->category->type) }}">
                          ...
                          <div class="portfolio-info">
                              ...
                              <div class="d-flex flex-wrap gap-1 justify-content-center mt-2">
                                <a href="{{ asset('images/templates/' . $template->preview_image) }}"
                                   title="{{ $template->name }}"
                                   data-gallery="portfolio-gallery-{{ $template->category->id }}"
                    ...
                    <div class="swiper-slide">
                      <div class="testimonial-item">
                        <p>
                          ...
                          <span>{{ Str::limit($item['message'], 150) }}</span>
                          <i class="bi bi-quote quote-icon-right"></i>
                        </p>
                        <img src="{{ asset('assets/img/testimonials/' . $item['image']) }}" class="testimonial-img" alt="">
                        <h3>{{ $item['name'] }}</h3>
                        <h4>{{ $item['role'] }}</h4>
                      </div>
                    </div>
